...
describe("getFilteredPackages", () => {
  ...
  it("should not include any package when only a non-package file changes", async () => {
    const logger = createLogger();

    const monorepo = new Monorepo("getFilterPackages-nonPackageFile");
    ...
    // A change to a file that lives outside every workspace package must NOT
    // silently expand the scope to every package; that is what repoWideChanges
    // is for (see the next test).
    await monorepo.commitFiles({
      "docs/some-doc.md": "test content",
    });
  ...
  it("should include all packages when a non-package file matches repoWideChanges", async () => {
    const logger = createLogger();

    const monorepo = new Monorepo("getFilterPackages-nonPackageFile-repoWide");
